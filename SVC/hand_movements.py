...
class HandMovements:
    ...
    def update_width_height(self, image_height, image_width):
        ...
        self.x_start_screen = int(
            (1 - self.screen_proportion) * self.camera_width / 2)
        self.y_start_screen = int(
            (1 - self.screen_proportion) * self.camera_height / 2)
        self.x_end_screen = int(
            (1 + self.screen_proportion) * self.camera_width / 2)
        self.y_end_screen = int(
            (1 + self.screen_proportion) * self.camera_height / 2)
    ...
    def execute_movement(self, pose, lm, delay, frame):
        ...
        if pose == 'left_click':
            ...
            if self.mouse_on_screen(x_mouse, y_mouse):
                ...
                x_average, y_average, idle = self.update_moving_average_xy(
                    x_mapper, y_mapper)
                ...
                if not idle:
                    ...
                    cv2.circle(frame, (x_average_cam, y_average_cam),
                               3, (255, 0, 0), 4)
                else:
                    cv2.putText(frame, f"Position locked", (30, 60),
